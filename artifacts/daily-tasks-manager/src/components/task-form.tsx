...
export function TaskForm({ date, task, initialCategory, categoryLocked = false, spaces, onClose }: TaskFormProps) {
  ...
  return (
    <div dir="rtl" className="fixed inset-0 z-50 flex items-end justify-center bg-foreground/35 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="presentation" onMouseDown={(event) => {
      if (event.target === event.currentTarget) onClose();
    }}>
      <div className="animate-rise max-h-[calc(100dvh-0.75rem)] w-full max-w-2xl overflow-y-auto overscroll-contain rounded-t-[1.7rem] border border-card-border bg-card text-card-foreground shadow-2xl sm:max-h-[calc(100dvh-3rem)] sm:rounded-[1.7rem]" role="dialog" aria-modal="true" aria-labelledby="task-form-title">
