...
function AccountControl() {
  ...
  return <div className="flex items-center gap-1 rounded-xl border border-border/80 bg-background/80 p-1 shadow-sm">
    <span className="hidden items-center gap-2 px-2 text-xs font-extrabold md:flex"><UserRound size={14}/>{name}</span>
    {adminAccess.data?.isAdmin && <a href={`${basePath}/admin`} className="rounded-lg bg-accent/10 px-3 py-2 text-xs font-extrabold" data-testid="link-admin"><ShieldCheck size={14} className="ml-1 inline"/>الإدارة</a>}
    <button type="button" onClick={() => void signOut({ redirectUrl: basePath || '/' })} className="rounded-lg bg-muted px-3 py-2 text-xs font-extrabold text-primary">خروج</button>
  </div>;
}
  ...
  return <div className="noise-overlay task-shell min-h-[100dvh]" dir="rtl">
    <header className="sticky top-0 z-30 border-b border-border/70 bg-card/90 backdrop-blur">
      <div className="mx-auto flex max-w-[1480px] flex-wrap items-center justify-between gap-3 px-5 py-3 sm:px-8 lg:px-12">
        <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary text-secondary"><Target size={21}/></div><div><p className="text-lg font-black">إنجازك اليومي</p><p className="text-[11px] font-semibold text-muted-foreground">اليوم أولاً</p></div></div>
    ...
    <main className="mx-auto max-w-[1480px] px-5 pb-12 pt-5 sm:px-8 lg:px-12">
      <section className="mb-5 grid gap-4 lg:grid-cols-[1fr_340px] lg:items-end">
        ...
        <div className="flex items-center justify-between rounded-xl border bg-card/75 p-2"><button onClick={() => setSelectedDate(shiftDate(selectedDate,-1))} className="rounded-lg p-3"><ChevronRight size={20}/></button><button onClick={() => setSelectedDate(dateKey(new Date()))} className="rounded-lg bg-primary px-4 py-2 text-sm font-extrabold text-primary-foreground">اليوم</button><input type="date" value={selectedDate} onChange={(e)=>setSelectedDate(e.target.value)} className="min-w-0 rounded-lg bg-background px-3 py-2 text-sm font-bold"/><button onClick={() => setSelectedDate(shiftDate(selectedDate,1))} className="rounded-lg p-3"><ChevronLeft size={20}/></button></div>
      </section>

      <div className="flex flex-col">
        {visibleSet.has('summary') && <section className="mb-5 rounded-xl border bg-card/70 p-4" style={sectionStyle('summary')}>
        ...
        {visibleSet.has('taskMap') && <section className="mb-5 border-y border-border bg-card/30 py-5" style={sectionStyle('taskMap')}>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div><h2 className="text-lg font-extrabold">خريطة اليوم</h2><p className="text-xs text-muted-foreground">اختار المساحة وشوف مهامها مباشرة</p></div>
            <div className="flex gap-2"><button onClick={()=>setShowSpaceForm(true)} className="rounded-lg border px-4 py-2 text-sm font-extrabold"><Plus size={16} className="ml-1 inline"/>مساحة</button><button onClick={()=>openNew()} className="rounded-lg bg-secondary px-4 py-2 text-sm font-extrabold"><Plus size={16} className="ml-1 inline"/>مهمة</button></div>
          </div>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6">
            <button onClick={()=>setActiveCategory('all')} className={`group relative min-h-[112px] border p-4 text-center transition ${activeCategory==='all'?'border-primary bg-primary text-primary-foreground shadow-sm':'border-border bg-background hover:border-primary/40 hover:bg-muted/40'} rounded-lg`}>
              <span className={`mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-full border ${activeCategory==='all'?'border-primary-foreground/30 bg-primary-foreground/10':'border-border bg-muted/50'}`}><Target size={20}/></span>
              <span className="block text-sm font-extrabold">الكل</span>
              <span className={`mt-1 block text-[11px] font-bold ${activeCategory==='all'?'text-primary-foreground/75':'text-muted-foreground'}`}>{tasks.length} مهمة</span>
            </button>
            {categories.map((category)=>{const meta=getSpaceMeta(category,spaces);const editable=realSpaces.find((s)=>s.name===category);const count=taskCounts[category]??0;return <div key={category} className="relative">
              <button onClick={()=>setActiveCategory(category)} className={`group min-h-[112px] w-full border p-4 text-center transition ${activeCategory===category?'border-primary bg-primary text-primary-foreground shadow-sm':'border-border bg-background hover:border-primary/40 hover:bg-muted/40'} rounded-lg`}>
                <span className={`mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-full border ${activeCategory===category?'border-primary-foreground/30 bg-primary-foreground/10':'border-border bg-muted/50'}`}><span className="h-3 w-3 rounded-full" style={{backgroundColor:meta.color}}/></span>
                <span className="block truncate text-sm font-extrabold">{category}</span>
                <span className={`mt-1 block text-[11px] font-bold ${activeCategory===category?'text-primary-foreground/75':'text-muted-foreground'}`}>{count} {count===1?'مهمة':'مهام'}</span>
              </button>
              {editable&&<div className="absolute left-1.5 top-1.5 flex gap-1 opacity-70 transition hover:opacity-100"><button onClick={()=>setEditingSpace(editable)} className="rounded-md bg-background/85 p-1.5 text-muted-foreground shadow-sm"><Pencil size={11}/></button><button onClick={()=>removeSpace(editable)} className="rounded-md bg-background/85 p-1.5 text-destructive shadow-sm"><Trash2 size={11}/></button></div>}
            </div>;})}
          </div>
        </section>}

        {visibleSet.has('tasks') && <section className="mt-1" style={sectionStyle('tasks')}>
          {taskQuery.isLoading ? <div className="grid gap-4 md:grid-cols-2"><div className="h-32 animate-pulse rounded-xl bg-muted"/><div className="h-32 animate-pulse rounded-xl bg-muted"/></div> : taskQuery.isError ? <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-10 text-center"><CircleAlert className="mx-auto mb-3 text-destructive"/><p className="font-extrabold">تعذر تحميل المهام</p><button onClick={()=>void taskQuery.refetch()} className="mt-4 rounded-lg bg-primary px-4 py-2 text-sm font-bold text-primary-foreground"><RefreshCw size={14} className="ml-1 inline"/>حاول مرة أخرى</button></div> : visibleTasks.length===0 ? <div className="rounded-xl border border-dashed p-12 text-center"><Sparkles className="mx-auto mb-3 text-primary"/><p className="font-extrabold">لا توجد مهام هنا</p><button onClick={()=>openNew(activeCategory==='all'?undefined:activeCategory)} className="mt-4 rounded-lg bg-primary px-5 py-2.5 text-sm font-extrabold text-primary-foreground">أضف مهمة</button></div> : <div className="space-y-5">{activeCategory!=='all'&&<QuickTaskInput date={selectedDate} category={activeCategory}/>} {categories.filter((category)=>activeCategory==='all'?visibleTasks.some((t)=>t.category===category):category===activeCategory).map((category)=>{const categoryTasks=visibleTasks.filter((t)=>t.category===category);const meta=getSpaceMeta(category,spaces);return <div key={category} className="border bg-card/55 p-4 rounded-xl" style={{borderInlineStartColor:meta.color,borderInlineStartWidth:4}}><div className="mb-3 flex items-center justify-between"><div><h3 className="font-extrabold">{category} <span className="mr-1 rounded-md bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">{categoryTasks.length}</span></h3><p className="text-xs text-muted-foreground">{meta.description}</p></div><button onClick={()=>openNew(category)} className="rounded-lg border p-2"><Plus size={16}/></button></div><div className="space-y-3">{categoryTasks.map((task)=><TaskCard key={task.id} task={task} date={task.taskDate} spaces={spaceNames} onDragStart={(id)=>setDraggedTaskId(id||null)} onDropTask={(draggedId)=>void reorderTasks(category,draggedId,task.id)} isDragging={draggedTaskId===task.id}/>)}</div></div>;})}</div>}
        </section>}
      </div>
    </main>
