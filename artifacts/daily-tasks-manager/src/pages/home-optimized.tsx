  ...
  return <div className="noise-overlay task-shell min-h-[100dvh]" dir="rtl">
    <header className="sticky top-0 z-30 border-b border-border/70 bg-card/90 backdrop-blur">
      <div className="mx-auto flex max-w-[1480px] flex-wrap items-center justify-between gap-3 px-5 py-3 sm:px-8 lg:px-12">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2"><PersistentSpaceLinks spaces={realSpaces}/><DashboardCustomizer/><AccountControl/></div>
      </div>
    </header>
    ...
    <main className="mx-auto max-w-[1480px] px-5 pb-12 pt-5 sm:px-8 lg:px-12">
      ...
      <div className="flex flex-col">
        ...
        {visibleSet.has('taskMap') && <section className="mb-5 rounded-3xl border border-card-border bg-card/70 p-4" style={sectionStyle('taskMap')}>
          ...
          <div className="flex flex-wrap gap-2"><button onClick={()=>setActiveCategory('all')} className={`rounded-xl border px-4 py-2 text-sm font-extrabold ${activeCategory==='all'?'bg-primary text-primary-foreground':'bg-background'}`}>الكل</button>{categories.map((category)=>{const meta=getSpaceMeta(category,spaces);const editable=realSpaces.find((s)=>s.name===category);return <div key={category} className="relative"><button onClick={()=>setActiveCategory(category)} className={`rounded-xl border px-4 py-2 pl-14 text-sm font-extrabold ${activeCategory===category?'bg-primary text-primary-foreground':'bg-background'}`}><span className="ml-2 inline-block h-2.5 w-2.5 rounded-full" style={{backgroundColor:meta.color}}/>{category}</button>{editable&&<><button onClick={()=>setEditingSpace(editable)} className="absolute left-7 top-1/2 -translate-y-1/2 p-1"><Pencil size={12}/></button><button onClick={()=>removeSpace(editable)} className="absolute left-1 top-1/2 -translate-y-1/2 p-1 text-destructive"><Trash2 size={12}/></button></>}</div>;})}</div>
        </section>}
