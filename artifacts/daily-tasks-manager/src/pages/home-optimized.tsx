...
const FocusOverdueTools = lazy(() => import('@/components/focus-overdue-tools').then((m) => ({ default: m.FocusOverdueTools })));
...
function SectionFallback() {
  return <div className="mb-6 h-28 animate-pulse rounded-3xl bg-muted/60" />;
}
...
function AccountControl() {
  ...
  const adminAccess = useGetAdminAccess({
    query: { enabled: Boolean(user?.id), staleTime: 5 * 60_000, refetchOnWindowFocus: false },
  });
...
function QuickTaskInput({ date, category }: { date: string; category: Category }) {
  ...
  return <form onSubmit={submit} className="flex gap-2 rounded-2xl border border-primary/20 bg-primary/[0.04] p-2.5">
    <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="اكتب مهمة سريعة..." className="h-11 min-w-0 flex-1 rounded-xl border border-input bg-background px-3 text-sm outline-none" />
    <button type="submit" disabled={createTask.isPending || !title.trim()} className="rounded-xl bg-primary px-4 text-sm font-extrabold text-primary-foreground disabled:opacity-50">{createTask.isPending ? <Loader2 size={16} className="animate-spin" /> : <Plus size={16} />}</button>
  </form>;
}
...
export default function HomeOptimized() {
  ...
  const preferences = useGetDashboardPreferences({ query: { staleTime: 5 * 60_000, refetchOnWindowFocus: false } });
  ...
  const spacesQuery = useListSpaces({ query: { staleTime: 5 * 60_000, refetchOnWindowFocus: false } });
  const taskQuery = useListTasks(range, { query: { staleTime: 30_000, refetchOnWindowFocus: false } });
  const summaryQuery = useGetTaskSummary(range, { query: { staleTime: 30_000, refetchOnWindowFocus: false } });
  const onboarding = useGetOnboardingStatus({ query: { staleTime: 5 * 60_000, refetchOnWindowFocus: false } });
  ...
  const removeSpace = (space: Space) => {
    deleteSpace.mutate({ id: space.id }, { onSuccess: () => {
      if (activeCategory === space.name) setActiveCategory('all');
  ...
  return <div className="noise-overlay task-shell min-h-[100dvh]" dir="rtl">
    <header className="border-b border-border/70 bg-card/60">
      <div className="mx-auto flex max-w-[1480px] flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-12">
        <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary text-secondary"><Target size={22} /></div><div><p className="text-xl font-black">إنجازك اليومي</p><p className="text-xs font-semibold text-muted-foreground">الأهم فقط، والباقي عند الطلب</p></div></div>
        <div className="flex flex-wrap items-center justify-end gap-2"><DashboardCustomizer /><PalettePicker /><AccountControl /></div>
      </div>
    </header>

    <main className="mx-auto max-w-[1480px] px-5 pb-12 pt-6 sm:px-8 lg:px-12">
      <div className="flex flex-col">
        {visibleSet.has('dateHeader') && <section className="mb-6 grid gap-5 lg:grid-cols-[1fr_360px] lg:items-end" style={sectionStyle('dateHeader')}>
          <div><div className="mb-2 flex items-center gap-2 text-sm font-bold text-muted-foreground"><CalendarDays size={17} className="text-primary" />{isToday ? 'اليوم' : 'يوم آخر'}</div><h1 className="text-3xl font-extrabold sm:text-5xl">{dateLabel(selectedDate)}</h1><p className="mt-3 text-sm text-muted-foreground">{getDailyMessage(selectedDate)}</p></div>
          <div className="flex items-center justify-between rounded-2xl border border-border bg-card/75 p-2"><button onClick={() => setSelectedDate(shiftDate(selectedDate, -1))} className="rounded-xl p-3"><ChevronRight size={20} /></button><button onClick={() => setSelectedDate(dateKey(new Date()))} className="rounded-xl bg-primary px-4 py-2 text-sm font-extrabold text-primary-foreground">اليوم</button><input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="min-w-0 rounded-xl bg-background px-3 py-2 text-sm font-bold" /><button onClick={() => setSelectedDate(shiftDate(selectedDate, 1))} className="rounded-xl p-3"><ChevronLeft size={20} /></button></div>
        </section>}
        ...
        {visibleSet.has('summary') && <section className="mb-6 grid gap-3 md:grid-cols-3" style={sectionStyle('summary')}>
          <div className="rounded-2xl bg-primary p-4 text-primary-foreground"><p className="text-sm font-bold opacity-70">الإنجاز</p><p className="mt-2 text-4xl font-extrabold">{completion}%</p></div>
          <div className="rounded-2xl border bg-card p-4"><ClipboardList className="text-accent" size={20}/><p className="mt-2 text-3xl font-extrabold">{summary?.remaining ?? '—'}</p><p className="text-xs text-muted-foreground">متبقي</p></div>
          <div className="rounded-2xl border bg-card p-4"><LayoutGrid className="text-primary" size={20}/><p className="mt-2 text-3xl font-extrabold">{summary?.total ?? '—'}</p><p className="text-xs text-muted-foreground">كل المهام</p></div>
        </section>}
        ...
        {visibleSet.has('focusTools') && <div style={sectionStyle('focusTools')}><Suspense fallback={<SectionFallback />}><FocusOverdueTools /></Suspense></div>}
