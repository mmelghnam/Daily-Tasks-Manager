...
Task action menus should be rendered in a viewport-level layer anchored to the options button, not as a simple absolute child of the task card. When the button is near the left edge in RTL, flip the menu to the button's right side instead of clamping it to the viewport edge. Portal menus also need pointer-leave, outside-click, and Escape dismissal.
